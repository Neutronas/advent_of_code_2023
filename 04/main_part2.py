...
match_counts_number = [1] * len(lottery_numbers)
...
def calculate_tickets(match_counts, match_counts_number):
    for card_number, matches in match_counts.items():
        print(f'Card number {card_number} has {matches} matches')
        for i in range(0, matches):
            for j in range(0, match_counts_number[card_number-1]):
                match_counts_number[card_number+i] += 1
    return match_counts_number

    
print(f"Total sum of points: {total_sum_of_points}")
match_counts_number = calculate_tickets(match_counts, match_counts_number)
print(sum(match_counts_number))
