...
mappingOfStrength = {'A' : 14, 'K': 13, 'Q': 12, 'T': 10, '9': 9, '8': 8, '7': 7, '6': 6, '5': 5, '4': 4, '3': 3, '2': 2, 'J': 1}
...
class CardObject:
    ...
    def findHandType(self):
        ...
        if(len(self.hand) == 5):
            ...
            if 'J' in self.hand[0] or 'J' in self.hand[1] :
                self.handType = HandTypes.FiveOfAKind
            else:
            ...
            if 'J' in self.hand[1] or 'J' in self.hand[2]:
                self.handType = HandTypes.FourofAKind
            else:
            ...
            if 'J' in self.hand[0] or 'J' in self.hand[1]:
                self.handType = HandTypes.FourofAKind
            elif('J' in self.hand[2]):
                self.handType = HandTypes.FullHouse
            else:
                self.handType = HandTypes.TwoPair
        elif(self.hand[0][1] == 2 and  'J' in self.hand[1]) or (self.hand[0][1] == 2 and 'J' in self.hand[2]) or (self.hand[0][1] == 2 and 'J' in self.hand[3]) or (self.hand[0][1] == 2 and 'J' in self.hand[0]):
            self.handType = HandTypes.ThreeofAKind
        else:
            self.handType = HandTypes.OnePair
